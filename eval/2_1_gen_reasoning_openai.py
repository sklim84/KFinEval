"""
추론 응답 생성 스크립트 (OpenAI API 사용)

이 스크립트는 OpenAI API를 사용하여 주어진 문맥과 질문에 대한 단계별 추론을 생성합니다.
...
def generate_reasoning(
    ...
    Returns:
        (생성된 추론 텍스트, raw_response JSON 문자열) 튜플
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if reasoning_config is not None:
                ...
                return reasoning_text.strip(), json.dumps(response.model_dump(), ensure_ascii=False)
            
            # reasoning_config가 None인 경우 (지원하지 않는 모델)
            else:
                raise ValueError(f"모델 '{model_name}'은 Responses API를 지원하지 않습니다. GPT-5 시리즈만 지원됩니다.")
